...
from keras.layers import (BatchNormalization, Conv1D, Dense, Input, 
    TimeDistributed, Activation, Bidirectional, SimpleRNN, GRU, LSTM, AveragePooling1D, MaxPooling1D)
...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    x = GRU(units_conv, activation='relu',
...
def final_model(input_dim, units, units_conv, dilation, filters, kernel_size, conv_stride,
    conv_border_mode,recur_layers, output_dim=29):
    ...
    for i in range(recur_layers):
        x = Bidirectional(GRU(units,
                              activation='relu',
                              return_sequences=True,
                              implementation=2,
                              dropout=0.2,
                              recurrent_dropout=0.4), merge_mode='concat')(x)
        x = BatchNormalization()(x)
        units += int(0.5 * units)
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(x)
