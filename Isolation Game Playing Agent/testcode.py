...
if __name__ == "__main__":
    ...
    print("\n" + game.to_string())
    game.apply_move((int(game.width/2), int(game.height/2)))
    print("\n" + game.to_string())
    ...
    totalboard = [(i, j) for i in range(7) for j in range(7)]
    totalsize = len(totalboard)
    currentboard = game.get_blank_spaces()
    currentsize = len(currentboard)

    print(currentboard)
    print([currentboard[i][1] for i in range(len(currentboard))])
    print([currentboard[i][0] for i in range(len(currentboard))])
    print("\n{}".format(totalsize - currentsize))

    k = game.get_player_location(game.active_player)
    print(k)

    for i in enumerate(currentboard):
        print(i[1])
