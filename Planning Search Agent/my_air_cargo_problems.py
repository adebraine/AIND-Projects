...
class AirCargoProblem(Problem):
    ...
    def get_actions(self):
        """
        This method creates concrete actions (no variables) for all
        actions in the problem domain action schema and turns them
        into complete Action objects as defined in the aimacode.planning
        module. It is computationally expensive to call this method directly;
        however, it is called in the constructor and the results cached in
        the `actions_list` property.
...
if __name__ == '__main__':
    run = True
    while run:
        pr = input("Choose problem number (1, 2, 3): ")
        try:
            pr = int(pr)
            if pr <= 3 and pr > 0:
                if pr == 1:
